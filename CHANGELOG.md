...
- 改进：WebUI 定时设置改版，新增时间点列表（如 `09:00`、`21:30`）和星期限制（`每天`、`周一至周五`、`1,3,5`），替代原生 cron 表达式，更直观易用。
- 兼容：老配置中的 `publish_cron` 等原始 cron 字符串仍保留，时间点列表为空时自动回退到旧值。
- 对接：日程数据源从 Life Scheduler 改为 Busy Schedule 插件，支持读取今日穿搭、日程、当前活动、下一个活动、自定义提示词。
- 占位符：图片提示词模板和配文模板新增 `{outfit}`、`{schedule}`、`{current_activity}`、`{next_activity}`、`{custom_prompt}` 占位符。
